...
//Lấy dữ liệu ra
$post_id = 0;
// cho = 0 rồi nên khỏi check rỗng chi nữa, cẩn thận nên mình check luôn
if(!empty($_GET['id'])){
    $post_id = $_GET['id'];
...
$post = get_post_by_id($post_id);
        ...
        $category_id = $post['category_id']; //Khỏi check
    ...
    if(!empty($_POST)){
        ...
        if($is_ok){
            //update bài viết
            $r = update_post($post_id,$post_title,$post_desc,$post_thumb,$post_content,$category_id);  //kt đk thêm thành công
        }
